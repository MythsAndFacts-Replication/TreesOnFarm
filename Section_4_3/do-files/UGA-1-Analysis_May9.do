...
******* GRAPHS 1: %HH For Different Type of crop (ALL TYPES OF TREES)
      ...
      qui estpost svy: tabulate `i', percent subpop(if farm_size>0 & season==`s')
        ...
        *gen the value
        ...
        graph bar percent, over(n_plots , label(labsize(small))) over(season, label(labsize(small)) relabel(1 "First Crop Season" 2 "Second Crop Season" )) ytitle("% HH",size(small))  blabel(bar, position(outside) format(%9.2f) color(black) size(vsmall) justification(left) ) $graph_cofing bar(1,color(gs9)) 

        cd "$graphs/"
        graph export UGA_HH_`i'.png, replace width(4147) height(2250) 
       restore
       }
...
******* GRAPHS 2: Percentage with plot
     ...
     gen plots= n_parcel_Tree_Fruit+n_parcel_NA+n_parcel_Plant+n_parcel_Tree_Agri+n_parcel_Tree_wood
        ...
        graph hbar share_Tree_Fruit  share_Plant share_Tree_Agri share_Tree_wood share_NA  [pweight=wgt10],   over(season, label(labsize(small)) relabel(1 `""1st Crop""Season"' 2 `"" 2nd Crop""Season"' )) ytitle("% of Farms/Plots",size(small))  blabel(bar, position(outside) format(%9.2f) color(black) size(vsmall) justification(left) ) $graph_cofing  legend (label  (1 Fruit Tree) label (2 Plant/Herb/Grass/Roots) label (3 Tree Crops) label  (4 Trees for timber and fuel-wood) label (5 NA) ) bar(1,color(cranberry))   bar(2,color(midgreen))  bar(3,color(olive)) bar(4,color(forest green)) bar(5,color(maroon))
          ...
          graph export UGA_plot_classification.png, replace width(4147) height(2250) 



******* GRAPHS 3: Type of Crops system

        label def season 1 "First Crop Season" 2 "Second Crop Season"
        ...
        graph pie share_inter`i' share_nointer`i' [pweight=wgt10] ,  plabel(_all percent, format(%9.2f)  )   pie(1, color(ebg) )  pie(1, color(ebblue) )   legend (label  (1 Inter-Cropped) label  (2 Mono-Cropping) )  graphregion(color(white))  legend( region( style(none)) cols(3)  size(small) forcesize )  by(season, note("") graphregion(color(white)))  legend( region( style(none)) cols(3)  size(small) forcesize )    

        graph export UGA_INTER_`i'.png, replace width(4147) height(2250) 
    }
